...
writer = SummaryWriter(log_dir="Diffusion_d1")
tags = ["train_loss", "data_error","learning_rate"]

pre_model_save_path = None
save_path = './model/FN0_d1_checkpoint100.pt'


n_train =150
u_0 = torch.from_numpy(np.load('Dataset/diffusion/64_64/u0s_d1.npy').reshape(200,64,64,3)).float()
u_T = torch.from_numpy(np.load('Dataset/diffusion/64_64/uTs_d1.npy').reshape(200,64,64,1)).float()
print(u_0.shape)
print(u_T.shape)
